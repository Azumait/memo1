...
def index():
    if session:
        ...
        cursor.execute(
            'SELECT * FROM azuma_fm1_bbs1 WHERE del_flg = %s and id = %s', (0, id))
        bbs1rows = cursor.fetchall()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            'SELECT DISTINCT category FROM azuma_fm1_bbs1 WHERE del_flg = %s and id = %s', (0, id))
...
def loginpost():
    ...
    if request.method == 'POST' and 'id' in request.form and 'password' in request.form:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            'SELECT * FROM azuma_fm1_user1 WHERE id = %s AND password = %s', (id, password))
...
def registerpost():
    ...
    if idcheck:
        ...
        value = (id, password, realname, nickname, gender, birth, email, tel, phone,
                 facebook, twitter, instagram, googleplus, tumblr, dribble, dream1, address)
...
def membermodpost():
    ...
    if not (id and password and passwordcheck and realname and nickname and gender and birth and email):
        ...
        value = (password, realname, nickname, gender, birth, email, tel, phone,
                 facebook, twitter, instagram, googleplus, tumblr, dribble, dream1, address, id)
        cursor.execute(query, value)
...
def memberoutpost():
    if request.method == 'POST' and 'id' in request.form and 'password' in request.form:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            'SELECT * FROM azuma_fm1_user1 WHERE id = %s AND password = %s', (session['id'], password))
        memberout = cursor.fetchone()
        if memberout:
            cursor = conn.cursor(pymysql.cursors.DictCursor)
            cursor.execute(
                'DELETE FROM azuma_fm1_user1 WHERE id = %s AND password = %s', (session['id'], password))
            session.pop('member', None)
            session.pop('id', None)
            session.pop('realname', None)
            session.pop('nickname', None)
    ...
    if img_size1 > 10:
        ...
        if img_file1 != '':
            ...
            value = (category, title, content,
                     nickname, img_file1, img_size1, no)
            cursor.execute(query, value)
            conn.commit()
            id = session['id']
            cursor = conn.cursor(pymysql.cursors.DictCursor)
